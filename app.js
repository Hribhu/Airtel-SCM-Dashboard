...
function readExcel(file) {
    ...
    reader.onload = function (e) {
        ...
        try {
            ...
            const isNonScmFile = NON_SCM.includes(sheetName) && (sheetData.Stock_Open_PO_Transit.length || sheetData.PO_Dump.length || sheetData.ASN_InTransit.length);
            if (isNonScmFile) {
                fullData = [];
                showToast(`Loaded SCM Insights data from "${sheetName}". No main SCM metrics available — upload a file with SCM_Data sheet for full dashboard.`, "info", 6000);
                processData(fullData);
                return;
            }

...
function readExcel(file) {
    ...
    reader.onload = function (e) {
        ...
        try {
            ...
            // If file only has "Master Sheet" (exported from this dashboard), load it into
            // Stock_Open_PO_Transit so the SCM Insights tab can display it.
            if (workbook.SheetNames.includes("Master Sheet") && !sheetData.Stock_Open_PO_Transit.length) {
                sheetData.Stock_Open_PO_Transit = readSheetFixedHeader(workbook.Sheets["Master Sheet"], 0);
            }

            // Prefer SCM_Data sheet if available.
            // Skip sheets that are purely export artifacts (Master Sheet, PO Dump exports)
            // to avoid feeding non-SCM columns into the main metrics engine.
            const SCM_SHEETS = ["SCM_Data", "Sheet1", "Data", "SCM Data"];
            const NON_SCM = ["Master Sheet", "PO Dump", "KPI Summary", "Inventory Risk Items",
                             "Vendor Risk Analysis", "Delayed Shipments", "Open PO", "Transit Stock",
                             "PO Orders", "PO Deliveries", "Consumption", "ASN In Transit",
                             "Stock Report", "Monthly Consumption", "SCM Raw Data",
                             // Underscore-named variants (actual sheet names in uploaded workbooks)
                             "PO_Dump", "ASN_InTransit", "Stock_Report", "Monthly_Consumption",
                             "Stock_Open PO_Transit", "Stock_Open_PO_Transit"];

            let sheetName = workbook.SheetNames.find(s => SCM_SHEETS.includes(s))
                || workbook.SheetNames.find(s => !NON_SCM.includes(s) && !Object.keys(sheetData).map(k => k === "Stock_Open_PO_Transit" ? "Stock_Open PO_Transit" : k).includes(s))
                || workbook.SheetNames[0];

            // If the chosen sheet is a known non-SCM sheet and we have SCM Insights data, use empty SCM data
            const isNonScmFile = NON_SCM.includes(sheetName) && (sheetData.Stock_Open_PO_Transit.length || sheetData.PO_Dump.length || sheetData.ASN_InTransit.length || sheetData.Stock_Report.length || sheetData.Monthly_Consumption.length);
            if (isNonScmFile) {
                fullData = [];
                const loadedSheets = Object.entries(sheetData).filter(([,v]) => v.length > 0).map(([k]) => k.replace(/_/g," ")).join(", ");
                showToast(`SCM Insights data loaded (${loadedSheets}). Health Score requires an SCM_Data sheet with columns like CurrentStock, SafetyStock, SLAStatus.`, "info", 8000);
                processData(fullData);
                return;
            }
            ...
            if (!fullData.length) {
                // Still run dashboard if we have any sheet data (SCM Insights)
                if (Object.values(sheetData).some(a => a.length > 0)) {
                    showToast("No SCM records in main sheet — showing available SCM Insights data.", "info", 5000);
                    processData([]);
                } else {
...
function runDashboard() {
    ...
    setStatus(
        fullData.length
            ...
            : "SCM Insights data loaded — no main SCM records found"
    );

    document.getElementById("fileInput").value = "";

    // Auto-switch to SCM Insights tab if file has no main SCM records but has SCM sheet data
    if (!fullData.length && Object.values(sheetData).some(a => a.length > 0)) {
        const scmBtn = document.querySelector('[data-tab="tab-scminsights"]');
        if (scmBtn) scmBtn.click();
    }
...
function updateKPIs(m) {
    ...
    const avg = m.total ? (m.totalDelayDays / m.total).toFixed(1) : "—";
    ...
    setText("efficiencyScore", eff !== null ? eff + "%" : "N/A");
}
    ...
    if (!m.total) {
        document.getElementById("summaryText").innerHTML = `
            <div style="text-align:center;padding:24px;color:var(--muted);">
                ...
                <div style="font-weight:700;font-size:16px;color:var(--text);">No SCM Data Records</div>
                <div style="font-size:13px;margin-top:6px;">The uploaded file does not contain SCM records (requires <strong>SCM_Data</strong> or <strong>Sheet1</strong> sheet with columns like <em>CurrentStock, SafetyStock, Vendor, SLAStatus</em>).<br>Check the <strong>SCM Insights</strong> tab for any loaded data.</div>
            </div>`;
        return;
    }
...
// Returns null when there is no SCM data loaded (so callers can show N/A).
...
function calcHealth(m) {
    ...
    if (!m.total) {
        // Reset gauge and all department scores to N/A
        gauge.querySelector("span").textContent = "N/A";
        gauge.style.background = "conic-gradient(#94a3b8 0deg, #94a3b8 0deg, #e5e7eb 0deg)";
        gauge.style.setProperty("--score", 0);
        document.getElementById("riskLevel").innerHTML =
            `⚪ No Data <small style="font-weight:400;font-size:14px;">(upload SCM_Data file)</small>`;
        ["inventoryHealth","procurementHealth","vendorHealth","logisticsHealth"].forEach(id => setText(id, "N/A"));
        ["hbox-inv","hbox-proc","hbox-vend","hbox-log"].forEach(id => {
            const el = document.getElementById(id);
            if (el) { el.classList.remove("hbox-good","hbox-warn","hbox-bad"); el.classList.add("hbox-warn"); }
        });
        return;
    }
...
function buildInsights(m) {
    if (!m.total) {
        document.getElementById("aiInsights").innerHTML = `<div style="color:var(--muted);text-align:center;padding:16px;">Upload a file with SCM_Data sheet to generate insights.</div>`;
        return;
    }
    const list = [];
...
function buildAlerts(m) {
    ...
    if (!m.total) {
        if (el) el.innerHTML = `<div class="alert alert-info">[INFO] No SCM data loaded. Upload a file with SCM_Data sheet to generate alerts.</div>`;
        return;
    }
    const html = [];
    const MAX = 40;
...
function buildRecommendations(m) {
    ...
    if (!m.total) {
        if (el) el.innerHTML = `<div class="recommendation">[INFO] No SCM data loaded. Upload a file with SCM_Data sheet to generate recommendations.</div>`;
        return;
    }
...
function dataQuality(m) {
    if (!m.total) {
        // Even without a SCM_Data sheet, we can still run quality checks on the loaded sheetData
        const allSheetRows = Object.values(sheetData).flat();
        if (!allSheetRows.length) {
            const critEl = document.getElementById("criticalFields");
            if (critEl) critEl.innerHTML = `<div style="color:var(--muted);text-align:center;padding:16px;">Upload a file with SCM_Data sheet to run data quality checks.</div>`;
            return;
        }
        // Compute completeness across all loaded sheets
        let totalCells = 0, missingCells = 0, dupes = 0;
        const seen = new Map();
        const fieldMissing = {};
        ...
        const completeness = totalCells > 0 ? Math.round(((totalCells - missingCells) / totalCells) * 100) : 0;
        // Patch into m so the display code below works
        m.completeness = completeness;
        m.dupes = dupes;
        m.missingCells = missingCells;
        m.total = allSheetRows.length; // temporarily set so display code runs
        m.fieldMissing = fieldMissing;
        // Render the DQ panel metrics
        ...
        if (compEl) { const val = compEl.querySelector(".dq-val"); if (val) { val.textContent = completeness + "%"; val.style.color = completeness >= 95 ? "var(--success)" : completeness >= 80 ? "var(--warning)" : "var(--danger)"; } }
        if (dupEl) { const v = dupEl.querySelector(".dq-val"); if (v) { v.textContent = dupes; v.style.color = dupes === 0 ? "var(--success)" : "var(--warning)"; } }
        if (misEl) { const v = misEl.querySelector(".dq-val"); if (v) { v.textContent = missingCells.toLocaleString(); v.style.color = missingCells === 0 ? "var(--success)" : "var(--danger)"; } }
        // Render critical field quality radar based on all sheet columns
        const critEl = document.getElementById("criticalFields");
        if (critEl) {
            // Map known critical fields to actual column names across our sheets
            const SHEET_CRITICAL_FIELDS = [
                { label: "Material", keys: ["Item", "Item_Code", "Description"] },
                { label: "Vendor", keys: ["Supplier", "Vendor Name", "Vendor"] },
                { label: "CurrentStock", keys: ["Quantity", "Stock"] },
                { label: "SafetyStock", keys: ["Safety Stock", "SafetyStock"] },
                { label: "SLAStatus", keys: ["PO DPE STATUS", "PR DPE STATUS", "SLA Status"] },
                { label: "DelayDays", keys: ["Transit_Age (Days)", "DelayDays"] },
                { label: "Region", keys: ["Circle Name", "Region", "Site"] },
            ];
            const critHtml = SHEET_CRITICAL_FIELDS.map(({ label, keys }) => {
                let bestPct = null;
                keys.forEach(k => {
                    if (fieldMissing.hasOwnProperty(k) || allSheetRows.some(r => r.hasOwnProperty(k))) {
                        const missing = fieldMissing[k] || 0;
                        const relevant = allSheetRows.filter(r => r.hasOwnProperty(k));
                        if (relevant.length) {
                            const pct = Math.round(((relevant.length - missing) / relevant.length) * 100);
                            if (bestPct === null || pct > bestPct) bestPct = pct;
                        }
                    }
                });
                const status = bestPct === null ? "absent" : bestPct === 100 ? "ok" : bestPct >= 90 ? "warn" : "bad";
                const icons = { ok: "✅", warn: "⚠️", bad: "❌", absent: "🔴" };
                const labels = { ok: "Complete", warn: bestPct + "% complete", bad: bestPct + "% — needs attention", absent: "Column absent" };
                return `<div class="crit-row"><span class="crit-icon">${icons[status]}</span><span class="crit-field">${escHtml(label)}</span><span class="crit-status crit-${status}">${labels[status]}</span></div>`;
            }).join("");
            critEl.innerHTML = critHtml;
        }
        m.total = 0; // restore so health score stays N/A
        return;
    }
    const compEl = document.getElementById("dqCompleteness");
    const dupEl = document.getElementById("dqDupes");
...
function topRisks(m) {
    ...
    list.innerHTML = risks.length === 0
        ? (fullData.length ? "<li>No risks detected</li>" : "<li>Upload a file with SCM_Data sheet to detect risks</li>")
...
function buildCharts(m) {
    if (!m.total) {
        // Show empty state on chart canvases
        ["riskChart","riskPieChart","vendorChart","inventoryChart","shipmentTrendChart","vendorSlaChart"].forEach(id => {
            const el = document.getElementById(id);
            if (!el) return;
            ...
            ctx.fillText("No SCM data — upload a file with SCM_Data sheet", el.width / 2, el.height / 2);
        });
        return;
    }
    destroyChart("risk");
...
function exportExcel() {
    ...
    const kpiData = [
        ...
        { Metric: "Health Score", Value: (() => { const v = document.getElementById("gauge").querySelector("span").textContent; return v === "N/A" ? "N/A" : v + "/100"; })() },
...
async function generatePDFReport() {
    ...
    [
        ...
        ["Health Score", (() => { const v = document.getElementById("gauge").querySelector("span").textContent; return v === "N/A" ? "N/A" : v + "/100"; })()],
...
function buildRegionMap() {
    ...
    if (!fullData.length) {
        // Just draw an empty map with zero risk
        try {
            const data = google.visualization.arrayToDataTable([["Province","Risk"],["Delhi",0],["Tamil Nadu",0],["West Bengal",0],["Maharashtra",0],["Madhya Pradesh",0]]);
            new google.visualization.GeoChart(document.getElementById("regionMap")).draw(data, {region:"IN",resolution:"provinces",displayMode:"regions",backgroundColor:"transparent",datalessRegionColor:"#ececec",colorAxis:{colors:["#22c55e","#facc15","#f97316","#dc2626"]}});
            const rs = document.getElementById("regionSummary");
            if (rs) rs.innerHTML = `<span style="color:var(--muted)">No SCM data — upload a file with SCM_Data sheet for regional risk analysis</span>`;
        } catch(e) { console.warn("Region map error:", e); }
        return;
    }
    fullData.forEach(row => {
...
function buildAsnInTransit() {
    ...
    if (!rows.length) {
        // Clear charts and show no-data state
        destroyChart("asnItemQty"); destroyChart("asnPoDpe"); destroyChart("asnPrDpe");
        const body = document.getElementById("asnBody");
        if (body) {
            body.innerHTML = `<div style="text-align:center;padding:32px;color:var(--muted);">
                <div style="font-size:32px;margin-bottom:8px;">📦</div>
                <div style="font-weight:600;">No ASN In Transit Data</div>
                <div style="font-size:13px;margin-top:4px;">Your uploaded file does not contain an <strong>ASN_InTransit</strong> sheet. Upload a file with this sheet to view data.</div>
            </div>`;
        }
        return;
    }

    // Restore chart canvas structure if it was previously replaced with no-data message
    const body = document.getElementById("asnBody");
    if (body && !document.getElementById("asnItemQtyChart")) {
        body.innerHTML = `<div class="charts" style="padding:0;margin-bottom:16px;">
            <div class="chart-box"><h3>Item vs Quantity</h3><canvas id="asnItemQtyChart" role="img" aria-label="ASN Item vs Quantity"></canvas></div>
            <div class="chart-box"><h3>Item vs PO DPE Status</h3><canvas id="asnPoDpeChart" role="img" aria-label="ASN Item vs PO DPE Status"></canvas></div>
            <div class="chart-box"><h3>Item vs PR DPE Status</h3><canvas id="asnPrDpeChart" role="img" aria-label="ASN Item vs PR DPE Status"></canvas></div>
        </div>`;
    }
...
function buildStockReport() {
    ...
    if (!rows.length) {
        ...
        if (tbody) tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:var(--muted);padding:20px">
            No Stock Report data — upload a file containing a <strong>Stock_Report</strong> sheet</td></tr>`;
        const chartEl = document.getElementById("stockItemQtyChart");
        if (chartEl) { const ctx = chartEl.getContext("2d"); ctx.clearRect(0, 0, chartEl.width, chartEl.height); }
        return;
    }
    _stockData = rows;
...
function buildMonthlyConsumption() {
    ...
    if (!rows.length) {
        ...
        if (tbody) tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:20px">
            No Monthly Consumption data — upload a file containing a <strong>Monthly_Consumption</strong> sheet</td></tr>`;
        const chartEl = document.getElementById("monthlyItemQtyChart");
        if (chartEl) { const ctx = chartEl.getContext("2d"); ctx.clearRect(0, 0, chartEl.width, chartEl.height); }
        return;
    }
    _monthlyData = rows;
...
function buildMasterSheet() {
    ...
    if (!rows.length) {
        ...
        if (tbody) tbody.innerHTML = `<tr><td colspan="11" style="text-align:center;color:var(--muted);padding:20px">
            No Master Sheet data — upload a file containing a <strong>Stock_Open PO_Transit</strong> sheet</td></tr>`;
        return;
    }
    _masterData = rows;
...
function renderMasterTable(rows) {
    ...
    if (!tbody) return;
    if (!rows.length) {
        tbody.innerHTML = `<tr><td colspan="11" style="text-align:center;color:#6b7280;padding:20px">No matching results</td></tr>`;
        return;
    }
    ...
    tbody.innerHTML = rows.slice(0, 300).map(r => {
        ...
        // Transit column: "Transit as of today" from the sheet
        const transit  = get(r, "Transit as of today", "Transit as of today ", "Transit");
        const trStock  = get(r, "Transit + Stock ", "Transit + Stock");
        const avgCons  = get(r, "Avg Consumption");
        const covTr    = get(r, "Month Coverage with Stock+Transit", "Month Coverage (Stock+Transit)");
        const covOp    = get(r, "Month Coverage with Stock+ Open Order", "Month Coverage (Stock+Open Order)");
...
// ── Master Sheet Export — Excel (all columns) ─
function exportMasterExcel() {
    const rows = _masterData || [];
    if (!rows.length) { showToast("No Master Sheet data to export.", "warn"); return; }
    const get = (r, ...keys) => {
        for (const k of keys) {
            if (r[k] !== undefined && r[k] !== "" && r[k] !== null) return r[k];
            const match = Object.keys(r).find(rk => rk.trim() === k.trim());
    ...
    const exportRows = rows.map(r => ({
        "Item Code": get(r, "Item Code ", "Item Code", "ItemCode"),
        "Vendor": get(r, "Vendor"),
        "AOP": get(r, "AOP"),
        "20-Jan to 31-May Ordering": get(r, "20-Jan to 31-May Ordering", "20-Jan to 31-May Ordering ", "20 Jan to 31 May Ordering", "20Jan-31May Ordering"),
        "%AOP Ordering": get(r, "%AOP Ordering"),
        "Stock": get(r, "Stock"),
        "Transit": get(r, "Transit as of today", "Transit as of today ", "Transit"),
        "Transit + Stock": get(r, "Transit + Stock ", "Transit + Stock"),
        "Avg Consumption": get(r, "Avg Consumption"),
        "Month Coverage (Stock+Transit)": get(r, "Month Coverage with Stock+Transit", "Month Coverage (Stock+Transit)"),
        "Month Coverage (Stock+Open Order)": get(r, "Month Coverage with Stock+ Open Order", "Month Coverage (Stock+Open Order)"),
    }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(exportRows), "Master Sheet");
    XLSX.writeFile(wb, "Master_Sheet_Export.xlsx");
    showToast("Master Sheet exported to Excel.", "success");
}

// ── Master Sheet Export — PDF (all columns) ─
function exportMasterPDF() {
    const rows = _masterData || [];
    if (!rows.length) { showToast("No Master Sheet data to export.", "warn"); return; }
    if (typeof window.jspdf === "undefined" && typeof jsPDF === "undefined") {
        showToast("PDF library not loaded. Please refresh and try again.", "error"); return;
    }
    ...
    const fmtN = v => { const n = parseFloat(v); return isNaN(n) ? String(v || "—") : Number(n.toFixed(2)).toLocaleString(); };
    const fmtP = v => { const n = parseFloat(v); if (isNaN(n)) return String(v || "—"); return (n < 2 ? (n * 100).toFixed(1) : n.toFixed(1)) + "%"; };
    ...
    doc.text("Airtel SCM  |  Master Sheet Report", mg, 13);
    doc.setFontSize(9); doc.setFont(undefined, "normal");
    doc.text("Generated: " + new Date().toLocaleString(), mg, 22);
    doc.setTextColor(30, 30, 30);

    const headers = ["Item Code", "Vendor", "AOP", "Jan-May Ord.", "%AOP Ord.", "Stock", "Transit", "Trans+Stock", "Avg Cons.", "MC S+T", "MC S+OO"];
    const colWidths = [28, 36, 14, 22, 16, 14, 16, 20, 20, 20, 20];
    let y = 36;
    const rowH = 7;
    const pageH = doc.internal.pageSize.getHeight() - 14;

    // Helper: draw header — backgrounds FIRST, then all text on top (prevents fill overlap)
    const drawMasterHdr = (yPos) => {
        doc.setFillColor(40, 40, 60); doc.setFontSize(7.5); doc.setFont(undefined, "bold");
        let hx = mg;
        headers.forEach((h, i) => { doc.rect(hx, yPos, colWidths[i], rowH, "F"); hx += colWidths[i]; });
        doc.setTextColor(255, 255, 255);
        hx = mg;
        headers.forEach((h, i) => {
            const maxCh = Math.floor(colWidths[i] / 1.8);
            doc.text(h.substring(0, maxCh), hx + 1.5, yPos + 5);
            hx += colWidths[i];
        });
    };
    ...
    rows.forEach((r, idx) => {
        ...
        if (idx % 2 === 0) { doc.setFillColor(248, 248, 252); doc.rect(mg, y, colWidths.reduce((a,b)=>a+b,0), rowH, "F"); }
        doc.setTextColor(30, 30, 30);
        x = mg;
        const vals = [
            String(get(r, "Item Code ", "Item Code", "ItemCode")),
            String(get(r, "Vendor")),
            String(get(r, "AOP")),
            fmtN(get(r, "20-Jan to 31-May Ordering", "20-Jan to 31-May Ordering ", "20 Jan to 31 May Ordering", "20Jan-31May Ordering")),
            fmtP(get(r, "%AOP Ordering")),
            String(get(r, "Stock")),
            fmtN(get(r, "Transit as of today", "Transit as of today ", "Transit")),
            String(get(r, "Transit + Stock ", "Transit + Stock")),
            fmtN(get(r, "Avg Consumption")),
            fmtN(get(r, "Month Coverage with Stock+Transit", "Month Coverage (Stock+Transit)")),
            fmtN(get(r, "Month Coverage with Stock+ Open Order", "Month Coverage (Stock+Open Order)")),
        ];
        vals.forEach((v, i) => {
            const maxCh = Math.floor(colWidths[i] / 1.7);
            doc.text(String(v).substring(0, maxCh), x + 1.5, y + 5);
            x += colWidths[i];
        });
        y += rowH;
    });

    doc.save("Master_Sheet_Export.pdf");
    showToast("Master Sheet exported to PDF.", "success");
}

// ── PO Dump Export — Excel ─────────────────────────
function exportPoDumpExcel() {
    const rows = _poDumpData || [];
    if (!rows.length) { showToast("No PO Dump data to export.", "warn"); return; }
    const getPD = (r, ...keys) => {
        for (const k of keys) {
            if (r[k] !== undefined && r[k] !== "" && r[k] !== null) return r[k];
            const match = Object.keys(r).find(rk => rk.trim() === k.trim());
    ...
    const exportRows = rows.map(r => ({
        "PO Number":              getPD(r, "PO Number", "PONumber", "PO_Number"),
        "PO Date":                getPD(r, "PO Date", "PODate", "PO_Date"),
        "Vendor Name":            getPD(r, "Vendor Name", "VendorName", "Vendor"),
        "Item Code":              getPD(r, "Item Code", "ItemCode", "Item_Code"),
        "Item Description":       getPD(r, "Item Description", "ItemDescription", "Description"),
        "Qty Ordered":            getPD(r, "Qty Ordered", "QtyOrdered", "OrderedQty"),
        "Qty Received":           getPD(r, "Qty Received", "QtyReceived", "ReceivedQty"),
        "Qty Cancelled":          getPD(r, "Qty Cancelled", "QtyCancelled", "CancelledQty"),
        "Unit Price (₹)":         getPD(r, "Unit Price", "UnitPrice", "Unit Price (₹)"),
        "Total Value w/o Tax (₹)":getPD(r, "Total Value w/o Tax", "TotalValue", "Total Value in Currency w/o Tax"),
    }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(exportRows), "PO Dump");
    XLSX.writeFile(wb, "PO_Dump_Export.xlsx");
    showToast("PO Dump exported to Excel.", "success");
}

// ── PO Dump Export — PDF ───────────────────────────
function exportPoDumpPDF() {
    const rows = _poDumpData || [];
    if (!rows.length) { showToast("No PO Dump data to export.", "warn"); return; }
    if (typeof window.jspdf === "undefined" && typeof jsPDF === "undefined") {
        showToast("PDF library not loaded. Please refresh and try again.", "error"); return;
    ...
    const fmtN = v => { const n = parseFloat(v); return isNaN(n) ? String(v || "—") : n.toLocaleString("en-IN", { maximumFractionDigits: 2 }); };
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(16); doc.setFont(undefined, "bold");
    doc.text("Airtel SCM  |  PO Dump", mg, 13);
    doc.setFontSize(9); doc.setFont(undefined, "normal");
    doc.text("Generated: " + new Date().toLocaleString() + "   |   Total Rows: " + rows.length, mg, 22);
    doc.setTextColor(30, 30, 30);

    const headers = ["PO Number", "PO Date", "Vendor", "Item Code", "Description", "Qty Ord", "Qty Rec", "Qty Can", "Unit Price (Rs)", "Total Val (Rs)"];
    const colWidths = [28, 22, 28, 28, 44, 16, 16, 16, 26, 27];
    let y = 36; const rowH = 7;
    const pageH = doc.internal.pageSize.getHeight() - 14;

    // Helper: draw header — backgrounds FIRST, then text on top (prevents fill overpainting)
    const drawPoDumpHdr = (yPos) => {
        doc.setFillColor(40, 40, 60); doc.setFontSize(7); doc.setFont(undefined, "bold");
        let hx = mg;
        headers.forEach((h, i) => { doc.rect(hx, yPos, colWidths[i], rowH, "F"); hx += colWidths[i]; });
        doc.setTextColor(255, 255, 255);
        hx = mg;
        headers.forEach((h, i) => {
            const maxCh = Math.floor(colWidths[i] / 1.9);
            doc.text(h.substring(0, maxCh), hx + 1, yPos + 5);
            hx += colWidths[i];
        });
    };
    ...
    doc.setFont(undefined, "normal"); doc.setFontSize(7); doc.setTextColor(30, 30, 30);
    rows.forEach((r, idx) => {
        if (y + rowH > pageH) {
            doc.addPage(); y = 8;
        ...
        x = mg;
        const vals = [
            truncate(getPD(r, "PO Number", "PONumber", "PO_Number"), 14),
            truncate(getPD(r, "PO Date", "PODate", "PO_Date"), 12),
            truncate(getPD(r, "Vendor Name", "VendorName", "Vendor"), 16),
            truncate(getPD(r, "Item Code", "ItemCode", "Item_Code"), 18),
            truncate(getPD(r, "Item Description", "ItemDescription", "Description"), 22),
            fmtN(getPD(r, "Qty Ordered", "QtyOrdered", "OrderedQty")),
            fmtN(getPD(r, "Qty Received", "QtyReceived", "ReceivedQty")),
            fmtN(getPD(r, "Qty Cancelled", "QtyCancelled", "CancelledQty")),
            fmtN(getPD(r, "Unit Price", "UnitPrice", "Unit Price (₹)")),
            fmtN(getPD(r, "Total Value w/o Tax", "TotalValue", "Total Value in Currency w/o Tax")),
        ];
        vals.forEach((v, i) => {
    ...
    doc.save("PO_Dump_Export.pdf");
    showToast("PO Dump exported to PDF.", "success");
}

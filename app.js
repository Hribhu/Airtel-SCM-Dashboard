...
// ── Master Sheet Export — Excel (Item Code + Month Coverage cols only) ─
function exportMasterExcel() {
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(exportRows), "Master Coverage");
    XLSX.writeFile(wb, "Master_Coverage_Export.xlsx");
    showToast("Master Sheet exported to Excel.", "success");
}

// ── Master Sheet Export — PDF (Item Code + Month Coverage cols only) ─
function exportMasterPDF() {
    ...
    doc.text("Airtel SCM  |  Master Sheet Coverage Report", mg, 13);
    doc.setFontSize(9); doc.setFont(undefined, "normal");
    doc.text("Generated: " + new Date().toLocaleString(), mg, 22);
    doc.setTextColor(30, 30, 30);

    const headers = ["Item Code", "Transit", "Month Coverage (Stock+Transit)", "Month Coverage (Stock+Open Order)"];
    const colWidths = [70, 35, 70, 70];
    let y = 36;
    const rowH = 7;
    ...
    const drawMasterHdr = (yPos) => {
        doc.setFillColor(40, 40, 60); doc.setFontSize(8); doc.setFont(undefined, "bold");
        let hx = mg;
        headers.forEach((h, i) => { doc.rect(hx, yPos, colWidths[i], rowH, "F"); hx += colWidths[i]; });
        doc.setTextColor(255, 255, 255);
        hx = mg;
        headers.forEach((h, i) => {
            const maxCh = Math.floor(colWidths[i] / 2.2);
            doc.text(h.substring(0, maxCh), hx + 2, yPos + 5);
            hx += colWidths[i];
        });
    };
    ...
    rows.forEach((r, idx) => {
        if (y + rowH > pageH) {
            doc.addPage(); y = 8;
            drawMasterHdr(y);
            doc.setFont(undefined, "normal"); doc.setFontSize(8); doc.setTextColor(30, 30, 30);
            y += rowH;
        }
        ...
        vals.forEach((v, i) => {
            const maxCh = Math.floor(colWidths[i] / 2.0);
            doc.text(String(v).substring(0, maxCh), x + 2, y + 5);
            x += colWidths[i];
        });
        y += rowH;
    });

    doc.save("Master_Coverage_Export.pdf");
